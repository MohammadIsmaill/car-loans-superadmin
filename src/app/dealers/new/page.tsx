...
export default function AddNewDealership() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await dealersAPI.create({
        name: formData.name,
      ...
      if (response.success) {
        router.push('/dealers/new/success');
      }
    } catch (err: unknown) {
      const error = err as { response?: { data?: { error?: { message?: string } } } };
      setError(error.response?.data?.error?.message || 'Failed to create dealer');
    } finally {
      setLoading(false);
    }
  };
